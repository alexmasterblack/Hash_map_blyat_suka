...
public:
    ...
    hash_map_iterator() noexcept : data(nullptr), status_table(0) {}
...
public:
    ...
    hash_map_const_iterator() noexcept : data(nullptr), status_table(0) {}
...
public:
     ...
     */
    explicit hash_map(const allocator_type& a) : alloc(a), hash_map() {}
    ...
    * @param  a  An allocator object.
    */
    hash_map(const hash_map& umap, const allocator_type& a) : alloc(a), data(alloc.allocate(umap.bucket_count())), status_table(other.status_table),
        count_buckets(other.count_buckets), non_empty_buckets(other.non_empty_buckets), hash(other.hash), equal(equal), max_load(other.max_load) {
        for (size_type i = 0; i < count_buckets; i++) {
            if (status_table[i] != 0)
                insert(std::make_pair(other.data[i].first, other.data[i].second));
        }
    ...
    */
    ...
    std::pair<iterator, bool> try_emplace(const key_type& k, _Args&&... args) {
        //piecewise_construct - значение передается в качестве первого аргумента для создания парного объекта
        //forward_as_turple - кортеж имеет элементы данных ссылки rvalue, когда rvalue используются в качестве аргументов, с lvalue также.
    ...
    */
    ...
    std::pair<iterator, bool> insert_or_assign(const key_type& k, _Obj&& obj) {
        ...
        } else return insert(std::make_pair(k, std::forward<_Obj>(obj)));
    }
    ...
    std::pair<iterator, bool> insert_or_assign(key_type&& k, _Obj&& obj) {
        ...
        if (element != end()) {
            element->second = obj;
            return std::make_pair(element, true);
        } else return insert(std::make_pair(std::move(k), std::forward<_Obj>(obj)));
    }
     ...
     */
    void clear() noexcept {
        //нужно стереть сами элементы, не трогая память
        for (; begin() != end(); ++begin()) {
            erase(begin());
        }
    ...
    mapped_type& operator[](key_type&& k) {
        auto element = find(k);
        if (element == end()) {
            insert(std::make_pair(std::forward<key_type>(k), mapped_type()));
        }
        return at(k);
    }
    //@}
